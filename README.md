...
**Solution:** an automated tool that summarizes traffic into one-page reports and applies statistical + heuristic detection to surface _potentially_ malicious behaviour — leaving the final judgement to an analyst (the same model used by real SOC tools like Suricata/Zeek).
...
│   ├── make_pdf.py               # Builds the print-ready PDF report
│   └── make_screenshot.py        # Dev-only: captures terminal output for README
│
...
│   ├── protocol_chart.png
│   ├── top_ports_chart.png
│   └── network_traffic_report.pdf   # Print-ready full report
│
...
├── .python-version                 # Project Python version (3.13)
├── README.md
└── requirements.txt
```
...
1. **Install Python** — download Python 3.8+ from [python.org](https://www.python.org/downloads/). Tick _"Add Python to PATH"_ while installing.
...
4. **Get a capture file** (choose either):
   - Use the included sample: `captures/traffic.pcapng` — no extra step needed, _or_
   - Make your own with Wireshark: start capture → browse the internet for a minute → stop and _Save As_ `.pcapng` into the `captures/` folder.
5. **Run the analyzer:**
   ```bash
   python scripts/analyzer.py
   ```
   _Use your own capture:_ `python scripts/analyzer.py --capture captures/my_capture.pcapng`
